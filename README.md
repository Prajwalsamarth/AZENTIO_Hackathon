...
Missing stage-1 data? `run.py`. Missing artifacts? `run_fraud.py fit`. Missing
Ollama? Nothing breaks — every triaged record falls back to a deterministic
verdict, and the run says so loudly.
...
| `batch` | Scores every transaction, writes all outputs | ~2min warm, ~5min cold |
...
Progress goes to **stderr**, results to **stdout**, so `single --id X | jq`
stays clean however chatty the run is.
...
**Read `fraud_report.json` with its own disclaimer in hand** — see
[Metrics are not accuracy](#metrics-are-not-accuracy).
...
> **It is a library and a CLI, not a network service.** Nothing listens on a
> port. If you need HTTP, wrap `process()` yourself.
...
A record is a plain dict — the columns of `cleaned_transactions.csv`. The fields
...
account and customer fields stay empty and missing-indicator features carry that
fact into the model — being an orphan is itself a signal. A record that fails
outright still returns a verdict (`is_fraud: false`, `confidence: 0.0`, "routed
for manual review") and logs an `ERROR` naming the record.
...
record gets the deterministic rule verdict. This is also the fallback path, so it
...
Read `finetune/artifacts/ft_report.json` and `ft_report_real.json` before
choosing the tuned model for anything real — it is a 1B model LoRA-tuned on 24
examples, and those reports are the only evidence of what that bought.
...
| Batch much slower than 2 minutes | Cold model | Expected on the first run; the model stays resident afterwards |
...
The slow leg is the ~15% of records that reach the SLM, so the progress line
carries a rate and an ETA:
